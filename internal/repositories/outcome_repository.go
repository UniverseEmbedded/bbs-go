...
func (r *outcomeRepository) Create(db *gorm.DB, t *models.Outcome) (err error) {
	err = db.Create(t).Error
	return
}

func (r *outcomeRepository) Update(db *gorm.DB, t *models.Outcome) (err error) {
	err = db.Save(t).Error
	return
}

func (r *outcomeRepository) Updates(db *gorm.DB, id int64, columns map[string]interface{}) (err error) {
	err = db.Model(&models.Outcome{}).Where("id = ?", id).Updates(columns).Error
	return
}

func (r *outcomeRepository) UpdateColumn(db *gorm.DB, id int64, name string, value interface{}) (err error) {
	err = db.Model(&models.Outcome{}).Where("id = ?", id).UpdateColumn(name, value).Error
	return
}
...
func (r *outcomeRepository) GetLatestByPollId(db *gorm.DB, pollId int64) *models.Outcome {
	return r.FindOne(db, sqls.NewCnd().Eq("poll_id", pollId).Eq("latest", true).Desc("id"))
}
